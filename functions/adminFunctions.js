...
exports.getAllUsers = async (req, res) => {
    try {
        ...
        res.status(500).json({
            error: 'Error fetching users: ' + (err ? err.message : 'Unknown error'),
        });
    }
};
...
exports.adminChangePassword = async (req, res) => {
    try {
        ...
        res.status(500).json({
            error: 'Error changing password: ' + (err ? err.message : 'Unknown error'),
        });
    }
};
...
exports.adminChangeEmail = async (req, res) => {
    try {
        ...
        res.status(500).json({
            error: 'Error updating email: ' + (err ? err.message : 'Unknown error'),
        });
    }
};
...
exports.grantAdminRights = async (req, res) => {
    try {
        ...
        res.status(500).json({
            error: 'Error granting admin rights: ' + (err ? err.message : 'Unknown error'),
        });
    }
};
...
exports.adminDeleteUser = async (req, res) => {
    try {
        ...
        res.status(500).json({
            error: 'Error deleting user: ' + (err ? err.message : 'Unknown error'),
        });
    }
};
